...
describe("RockPaperScissorsv2 Unit tests", function () {
  ...
  it("Player 1 commits choice with 1000 dai", async function () {
    ...
    await RPS1.login(password);

    //act
    await expect(RPS1.player1Commit(Choice.ROCK, bet, password)).to.emit(RPS, "Player1Commited").withArgs(gameId);
    ...
    await expect((await RPS1.commits(player1.address, gameId)).toString()).to.equal("0xc3281f7ded705e5487c6d21ef2da15bb73847ec322e9a3d39a5482c154e5f3d6");
  ...
  it("player 1 and 2 commit with 1000 dai, then they both reveal, it is a tie, player 1 executes prize distribution, there is no balance to distribute", async function () {
    ...
    expect(await RPS.status(gameId)).to.equal(GameStatus.FINISHED);
